...
# External TMDB / IMDB poster import safely handled
try:
    from plugins.Dreamxfutures.Imdbposter import get_movie_detailsx, fetch_image, get_movie_details
except ImportError:
    get_movie_detailsx = None
    fetch_image = None
...
def generate_movie_message(movie_doc: dict, base_name: str) -> str:
    ...
    qualities = sorted(list({f["quality"] for f in files if f.get("quality") and f["quality"] != "N/A"}))
    languages = sorted(list({f["language"] for f in files if f.get("language") and f["language"] != "N/A"}))
    
    qual_str = ", ".join(qualities) if qualities else "HDRip"
    ...
    caption = (
        f"<b>🎬 Title:</b> <code>{base_name.title()}</code>\n"
        f"<b>🎭 Genre:</b> {genres}\n"
        f"<b>⭐️ Rating:</b> {rating}\n"
        f"<b>🔊 Audio:</b> {lang_str}\n"
        f"<b>💿 Quality:</b> {qual_str}\n"
        f"<b>📡 OTT:</b> {ott}\n\n"
        f"<b>🏷 Tags:</b> {tag}"
    )
    return caption
...
async def _process_with_lock(bot, filename, caption, media_info, base_name, processed):
    ...
    if not movie_doc:
        details = {}
        if TMDB_POSTER and get_movie_detailsx:
            ...
            if details.get("error") or (not details.get("poster_url") and not details.get("backdrop_url")):
                ...
                details = await get_movie_details(base_name) if get_movie_details else {}
        elif get_movie_details:
...
async def send_movie_update(bot, base_name):
    ...
    for attempt in range(max_retries):
        try:
            ...
            if movie_doc.get("poster_url") and not LINK_PREVIEW and fetch_image:
